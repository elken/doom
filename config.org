...
Now that I've moved to vertico, I'm quite enjoying the window being at the bottom so this will likely only stay for exwm.
...
Again, the relies on ivy which is no longer in use. If I ever go back to linux, this might be looked into but for now it's dead code
...
Shows colours as colours, like #fe132a
