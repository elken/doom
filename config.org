...
** Lua
I'm not a fan of lua, but until I can be bothered enough to port lua config to fennel, we're stuck with it.

First things first; we need a project mode for my awesomewm config so we get proper LSP results.
...
(add-hook! '+awesome-config-mode-hook
  (defun +awesome-config-mode-lsp-h ()
    (require 'ht)
    (setq lsp-lua-workspace-library (ht ("/usr/share/awesome/lib" t))
          lsp-lua-diagnostics-globals ["awesome" "client" "tag" "screen"])))
#+end_src
