...
(after! reaper
  (when (and (not (string= "" reaper-api-key))
             (not (string= "" reaper-account-id)))
...
(setq  doom-font (font-spec :family "Iosevka Nerd Font Mono" :size 13)
       ...
       doom-unicode-font (font-spec :family "Iosevka Nerd Font Mono" :size 14)
...
(custom-theme-set-faces! 'doom-nord
  ...
  `(corfu-default :background nil :inherit tooltip :extend t))
#+end_src
...
** Prettify org
**** Org-modern
Minad is a treasure that must be kept alive at all costs. =org-modern= is actually
quite a bit nicer than =org +pretty=, so we use that instead.
...
(use-package! org-modern
  ...
  (setq org-modern-star ["› "]
        ...
        '(("TODO" :inverse-video t :inherit org-todo)
          ...
          ("STORY" :inverse-video t :inherit +org-todo-project)
          ("INPROG" :inverse-video t :inherit +org-todo-active)
...
(unpin! eglot)
(package! org
  :pin "5766ddd758f6c2b498ddf4fb79f20f01a09674cc")
#+end_src
...
(setq ispell-program-name "aspell"
      ...
      cape-dict-file (if (file-exists-p ispell-personal-dictionary) ispell-personal-dictionary cape-dict-file))
#+end_src
