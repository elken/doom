...
*** Performance-related
Minor tweaks to improve overall performance
...
** Tabs
Often jump back and forth between wanting this and not.
...
(use-package! org
  ...
  :hook (org-mode . elken/org-font-setup))
#+end_src
...
** Font setup
Font setup to prettify the fonts. Uses IBM Plex Sans in most places except where
it makes sense to use the defined fixed width font.

Also need to investigate Overpass as a replacement for IBM Plex Sans.
...
** Variable pitch
Enable =variable-pitch-mode= to enable the more readable fonts where it makes sense
...
(use-package! org-variable-pitch
  ...
  :hook (after-init-hook . org-variable-pitch-setup))
#+end_src
...
** PHP
*** Module improvements
Adding here until [[https://github.com/hlissner/doom-emacs/pull/5292][#5292]] is merged.
...
(package! laravel-mode
  :recipe (:local-repo "~/build/lisp/laravel-mode"
           :build (:not compile)))
#+end_src
...
(use-package! laravel-tinker
  ...
  (map! :localleader
         :map php-mode-map
         :desc "Toggle a project-local Tinker REPL" "o t" #'laravel-mode-tinker-toggle))
#+end_src
