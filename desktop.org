...
(defun elken/exwm-update-title ()
  ...
  (pcase (downcase (or exwm-class-name ""))
    ...
    ("spotify" (exwm-workspace-rename-buffer (format "Spotify: %s" (elken/playerctl-format "metadata" "{{ artist }} - {{ title }}"))))))
  ...
  (doom-modeline-def-segment now-playing
    (let ((player (elt now-playing-text 0))
      ...
      (concat
       ...
       status
       (doom-modeline-spc)
       (propertize (truncate-string-to-width text 50 nil nil "...") 'face 'bold))))
